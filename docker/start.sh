...
# This image runs MariaDB in the same container as Apache/PHP.
# Do not depend on Render's external database variables for the bundled DB.
mkdir -p /run/mysqld
...
# PHP connects through TCP (127.0.0.1). Configure both localhost and
# 127.0.0.1 root accounts so MariaDB authentication cannot fall back to
# the Debian socket-only root configuration.
mariadb -uroot <<SQL
