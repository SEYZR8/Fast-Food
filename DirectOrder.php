...
$name=clean('name'); $number=preg_replace('/[^0-9+]/','',clean('number')); $email=clean('email'); $address=clean('address'); $orderName=clean('order'); $additional=clean('additional'); $qty=(int)(clean('qty')!==''?clean('qty'):clean('quantity')); $proId=clean('p_id'); $catId=clean('category'); $dateInput=clean('date');
...
$total=$price*$qty;$extra=$additional!==''?' Qo‘shimcha: '.$additional:'';
// Mirror the live order into the unified admin/courier queue.
$conn->query("CREATE TABLE IF NOT EXISTS admin_orders (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY, source_table VARCHAR(128) NULL, source_key VARCHAR(191) NULL, customer_name VARCHAR(190) NOT NULL DEFAULT '', phone VARCHAR(60) NOT NULL DEFAULT '', address TEXT NULL, note TEXT NULL, amount DECIMAL(12,2) NOT NULL DEFAULT 0, status VARCHAR(40) NOT NULL DEFAULT 'new', courier_id BIGINT UNSIGNED NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, UNIQUE KEY source_ref(source_table,source_key)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
$note='Mahsulot: '.$product['p_title'].' × '.$qty.$extra; $st=$conn->prepare("INSERT INTO admin_orders(source_table,source_key,customer_name,phone,address,note,amount,status) VALUES('card',?,?,?,?,?,?,'new') ON DUPLICATE KEY UPDATE customer_name=VALUES(customer_name),phone=VALUES(phone),address=VALUES(address),note=VALUES(note),amount=VALUES(amount)");$st->bind_param('sssssd',$invId,$name,$number,$address,$note,$total);$st->execute();
// Ensure a guest/customer profile can later use the same order history.
...
echo '<div class="alert alert-success" role="alert"><strong>Buyurtma qabul qilindi!</strong><br>Buyurtma: '.htmlspecialchars($invId).' · '.htmlspecialchars($product['p_title']).' × '.(int)$qty.'<br>Jami: '.number_format($total,0,'.',' ').' · Manzil: '.htmlspecialchars($address).htmlspecialchars($extra).'<br>Buyurtmangiz hozir admin paneldagi jonli navbatga tushdi. Tez orada siz bilan <a href="tel:+998998305603">+998 99 830 56 03</a> orqali bog‘lanamiz.</div>';
?>